<template>
    <section class="shop-header">
        ...
        <nav class="shop-nav" :style="{backgroundImage: `url(${info.bgImg})`}">
            <!-- 编程式跳转,@click="$router.back()" 
            但是需要此页面其他路由使用replace来跳转其他路由地址 -->
            <button class="back" @click="$router.back()">
                <i class="iconfont icon-arrow_left"></i>
            </button>
            <!-- 以下写法有重大bug
            路由(该路由keep-alive)跳转页面后; 但点击回退返回后,该路由却重新进行请求发送,页面重加载!!! -->
    ...
    export default {
        ...
        computed: {
            ...
            ...mapState({
                ...
                info: state => state.shop.shop.info || {}   // 第一个shop为模块,第二个shop为模块内的state状态数据
            })
        }
    }
</script>
