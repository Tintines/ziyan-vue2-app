    ...
    export default {
        ...
        methods: {
            ...
            _initScroll() {
                /* BScroll响应两次问题 */
                /* 判断语句
                当退出路由又进入同一路由时(由于路由重定向,每次进入都会显示goods),由于之前非路由数据缓存到vuex,
                使得数据没变化,导致初始化时调用的new BScroll 和 用来切换路由数据未变重新new BScroll;同时响应触发的bug!
                区别: ratings 和 info组件则不需要此判断 */
                if(!this.leftScroll) {
                    /* 左侧列表初始化滑动 */
                    /* 单一实例化: 
                ...
                } else {  // 当new BScroll 存在时,只进行重新计算刷新
        ...
        computed: {
            // ...mapState(['goods']),      // 非vuex时写法
            ...mapState({
                goods: state => state.shop.shop.goods || []  // 初始化页面显示
            }),
        ...
        mounted () {
            /* 解决在数据存在,路由切换导致BScroll滑动失效的bug */
            // 如果数据已经有了, 直接重新做初始化的操作
