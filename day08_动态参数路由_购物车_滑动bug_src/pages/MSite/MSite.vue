        ...
        <div class="msite_shop_list">
            ...
            <div class="shop_container">
                <ul class="shop_list_ul" v-if="shops.length">
                    <!-- 未指定特定商家时写法,没有进行params传参请求对应的指定商家 -->
                    <!-- <li class="shop_list_li" v-for="shop in shops" :key="shop.id" @click="$router.push('/shop')"> -->
                    <li class="shop_list_li" v-for="shop in shops" :key="shop.id" @click="$router.push(`/shop/${shop.id}`)">
                        <!-- 
                            传参方法一: 
                                可不起名字, 此时路由中配置的重定向自动跳转有效!! 
                                若调转后是多路由,且又有路由重定向时,只能使用该字符串携带params参数的形式
                                @click="$router.push(`/shop/${shop.id}`)"

                            传参方法二: 
                                需要给路由起名字,使用对象形式传参 此时路由中若配置的重定向功能,则该重定向自动跳转失效
                                @click="$router.push({
                                    name: 'shop',
                                    params: {
                                    id: shop.id
...
export default {
    ...
    computed: {
        ...
        /* vuex未模块化编码 对象 写法*/
        // ...mapState({address: 'address', categorys: 'categorys', shops: 'shops')}
        /* vuex模块化后写法 */
        /* 模块化编码后,经过总state找模块,总state中没有address(在模块msite中) */
        ...mapState({
            address: state => state.msite.address,      // state是总state
